...
const Service = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add the appropriate field based on service type
      if (selectedService.satuan === 'kg') {
        orderData.berat = parseFloat(formData.quantity);
      } else {
        orderData.jumlah = parseInt(formData.quantity);
      }
                ...
                <div className="card-body p-4">
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <span className={`h6 fw-bold text-${service.warna || 'primary'} mb-0`}>
                      Rp {service.harga?.toLocaleString('id-ID')} / {service.satuan || 'kg'}
                    </span>
                    <button 
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-4">
                    <label className="block text-gray-700 mb-2">
                      {selectedService.satuan === 'kg' ? 'Berat (kg)*' : 'Jumlah (pcs)*'}
                    </label>
                    <input
                      type="number"
                      name="quantity"
